...
```{r collecion, warning=FALSE,message=FALSE,prompt=TRUE}
require(msigdbr)

# Show supported gene sets colections
print(msigdbr::msigdbr_collections(),n=30)
```
```{r scgsea, warning=FALSE,message=FALSE,prompt=TRUE,results='hide'}
...
```{r graph, fig.cap="Cell Network. The cell network reconstructed by phenograph.",echo=TRUE,cache=TRUE,warning=FALSE,message=FALSE,prompt=TRUE,results='hide'}
require(netbiov)

# Exstract the graph and plot it
g = data$scgsea$cell.graph
g = igraph::simplify(data$scgsea$cell.graph)
