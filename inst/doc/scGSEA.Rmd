...
# Create t-UMAP space
data <-runReduction(data = data,reduction = "tumap",nt = 2,verbose = T,n_neighbors=150)
...
Now that we have reconstructed pathway's activity at single cell level we can try to cluster cell according to that. `gficf` package implement phenograph algorithm. Phenograph is a clustering method designed for high-dimensional single-cell data analysis. It works by first creating a graph ("network") that represents phenotypic similarities among cells by calculating the Jaccard coefficient between nearest-neighbor sets, and then identifying communities using the [Louvain method](https://sites.google.com/site/findcommunities/) in the recostructed graph.
...
## How can I use a custom gene set list?
...
## Where is the cell graph produced by phenograph?
The cell graph produced by phenograph algorithm is stored into the `data$scgsea$cell.graph` igraph object. That can be then plotted and manipulated with any other network package. Here we use [`netbiov`](https://www.bioconductor.org/packages/release/bioc/html/netbiov.html) package to visualize this network. Remember that in this weighted network the edge weights represents similarities among cells in terms of jaccard coefficients (i.e., they are not distances), so remember to transform these similarities into distances before to apply the minimum spanning tree (MST) algorithm on this network (see example below).

```{r stats,cache=TRUE,warning=FALSE,message=FALSE,prompt=TRUE}
require(igraph)

# Extract the graph and print infos
...
summary(g)
```
```{r graph, fig.cap="Cell Network. The cell network reconstructed by phenograph using the reconstructed pathway's activity levels in each cell. Bolder edges are edges along the MST path.",echo=TRUE,cache=TRUE,warning=FALSE,message=FALSE,prompt=TRUE,results='hide'}
require(netbiov)
